...
true <<COMMENTS
This script is a wrapper that evaluates pretrained models for each embedding setting:
  - Parses command-line arguments (e.g., --run_name, --ckpt, --config, --outdir).
  - Validates the existence of required files (e.g., checkpoint, config, evaluation scripts).
  - Runs the evaluation pipeline using evaluate_on_walle.py.
  - Calculates and summarizes metrics using calculate-mean-metrics.py.
  - Saves the summary metrics in a JSON file.
COMMENTS
...
[[ ! -f "$BASE/evaluate_on_walle.py" ]] && errMessages+=("evaluate_on_walle.py exists ... no")
...
# ----------------------------------------
# Evaluate the model
python3 inference/evaluate_on_walle.py \
  -o $outDir/$name \
  -c $config \
  -m $ckpt >$outDir/${name}-evaluate.log

# Calculate avg metrics
python3 inference/calculate-mean-metrics.py \
  ${outDir}/${name} -json ${outDir}/${name}-summary.json


####### test ########
true << COMMENTS
./inference/evaluate.sh -n likely-brook-1 
-m ~/Documents/GSU/Projects/Antibody-Design/epitope-prediction/results/asep/assets/likely-brook-1/best_k_models/rank_0-epoch_9.pt \
-c ~/Documents/GSU/Projects/Antibody-Design/epitope-prediction/results/asep/assets/likely-brook-1/config/config.yaml \
-o ~/Documents/GSU/Projects/Antibody-Design/epitope-prediction/results/asep/metrics/
COMMENTS