"""
This script evaluates the pretrained model on the test set (or validation/train set, depending on the input arguments). It performs the following tasks:
    - Loads the model checkpoint and configuration file.
    - Initializes the model with the trained weights.
    - Loads the dataset (e.g., AsEPv1Dataset) using the provided configuration.
    - Creates data loaders for the test/validation/train set.
    - Runs inference on the dataset using the trained model.
    - Computes predictions for bipartite graph edges (edge_index_bg_pred) and epitope residues (epitope_pred).
    - Computes metrics for: 
        - Epitope Residue Prediction: Metrics like MCC, ROC-AUC, precision, recall, F1-score.
        - Bipartite Graph Edge Reconstruction: Metrics like MCC, ROC-AUC, precision, recall, F1-score.
    - Saves the results (e.g., predictions, metrics) in JSON files.
"""
import sys, os
sys.path.append(os.path.abspath(os.path.join(os.getcwd(), '..')))

### when there is module import error, set the python environment var globally
# export PYTHONPATH=/Users/mansoor/Documents/GSU/Projects/Antibody-Design/epitope-prediction/epitope-pred/asepcode:$PYTHONPATH
