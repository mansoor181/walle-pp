...
class ABConfig(BaseConfig):
    ...
    def adjust_embedding_model_by_src(cls, values):
        ...
        # If src.script_path or src.method_name is not None, set embedding_model to custom
        if src.script_path is not None or src.method_name is not None:
            # Use a temporary variable to avoid recursion
            values_copy = values.model_copy(update={"embedding_model": "custom"})
            return values_copy
        return values
...
class EmbeddingConfig(BaseModel):
    ...
    def adjust_embedding_model(cls, values):
        ...
        if values.ab.embedding_model == "custom" or values.ag.embedding_model == "custom":
            """
            If `embedding_model` of either `ab` or `ag` is set to custom,
