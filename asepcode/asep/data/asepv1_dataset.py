...
class AsEPv1Dataset(PygInMemoryDataset):
    ...
    def __init__(
        ...
        # elif nft == "custom":
        #     print("Custom embeddings file found, reading from pt file...")
        #     print("Paths: ", self.processed_paths)
        ...
        if nft == "one_hot":
            ...
            # Generate custom embedding file name based on the embedding methods
            custom_emb_file = self._get_custom_embedding_filename()
            custom_emb_path = osp.join(self.processed_dir, custom_emb_file)

            if osp.exists(custom_emb_path):
                print(f"Reading from custom embedding file: {custom_emb_file}")
                self.data, self.slices = torch.load(custom_emb_path)
            else:
                print(f"Custom embedding file not found. Generating embeddings...")
                self.process()  # Generate embeddings
                # torch.save((self.data, self.slices), custom_emb_path)
                print(f"Saved custom embeddings to: {custom_emb_path}")
        else:
            raise NotImplementedError(
                f"node_feat_type={nft} is not implemented, valid options are: ['one_hot', 'pre_cal', 'custom']"
            )

    def _get_custom_embedding_filename(self) -> str:
        # print(self.emb_config)
        """Generate the custom embedding file name based on the embedding methods."""
        ab_method = self.emb_config.ab.custom_embedding_method_src.name
        ag_method = self.emb_config.ag.custom_embedding_method_src.name
        return f"{ab_method}_{ag_method}_emb.pt"
...
def get_node_feat_from_interim_graph_data(
    interim_graph_data: Dict[str, Any], embedding_config: EmbeddingConfig, abdbid
) -> Tuple[Tensor, Tensor]:
    """
    ...
    if (t := cfg.node_feat_type) != "custom":
        ...
        # if esm-if, then use pdb structures to generate embeddings
        if cfg.ab.custom_embedding_method_src == "embed_esm_if": 
            x_b = torch.cat(
                [
                    cfg.ab.custom_embedding_method(interim_graph_data["seqres"]["ab"]["H"], abdbid, "H"),
                    cfg.ab.custom_embedding_method(interim_graph_data["seqres"]["ab"]["L"], abdbid, "L"),
                ],
                dim=0,
            )
        else:
    ...
    # print("generating node embeddings... of size: ", x_b.shape)
    # ----------------------------------------
    # antigen
    # ----------------------------------------
    if (t := cfg.node_feat_type) != "custom":
        ...
        if cfg.ag.custom_embedding_method_src == "embed_esm_if":
            cfg.ag.custom_embedding_method(list(interim_graph_data["seqres"]["ag"].values())[0], abdbid)
        else:
            x_g = cfg.ag.custom_embedding_method(
                list(interim_graph_data["seqres"]["ag"].values())[0]
            )

    # print(f"x_b shape before filtering: {x_b.shape}")
    # print(f"seqres2cdr shape: {interim_graph_data['mapping']['ab']['seqres2cdr'].shape}")
    # x_b = x_b[(interim_graph_data["mapping"]['ab']['seqres2cdr']).astype(bool)]
    # print(f"x_b shape after filtering: {x_b.shape}")

    # # TODO: should we also provide the full version?
    # # use the binary vector to slice the embeddings

    # # Skip the filtering step for custom embeddings
    x_b = x_b[(interim_graph_data["mapping"]['ab']['seqres2cdr']).astype(bool)]
...
def convert_one_interim_to_pyg_pair_data(
    ...
    x_b, x_g = get_node_feat_from_interim_graph_data(
        interim_graph_data=data, embedding_config=embedding_config, abdbid=abdbid
    )
...
# #             dim=0,
# #         )
# #         print(f"x_b shape (custom embedding): {x_b.shape}")  # Debugging statement
...
# #             list(interim_graph_data["seqres"]["ag"].values())[0]
# #         )
# #         print(f"x_g shape (custom embedding): {x_g.shape}")  # Debugging statement

# #     # Skip the filtering step for custom embeddings
# #     if cfg.node_feat_type != "custom":
# #         x_b = x_b[(interim_graph_data["mapping"]['ab']['seqres2cdr']).astype(bool)]
# #         x_g = x_g[(interim_graph_data["mapping"]['ag']['seqres2surf']).astype(bool)]
